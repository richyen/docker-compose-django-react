...
import { Grid, Header, List, Segment, Container, Button } from 'semantic-ui-react';
...
{"section": "Contact Us",
 ...
 {"section": "About",
 "links": ["Our History", "Mission + Values"],
 "width": 3},
...
const footerLinks = (linkArr) => {
  return (
    linkArr.map((footerLink, index)=>{
      return (
        <List.Item as="a" key={index}>{footerLink}</List.Item>
      )
    })
  )
}
...
const Footer = () => {
  return (
    <Segment inverted vertical style={style.footerContainer}>
      <Container>
        <Grid inverted stackable textAlign="left" style={{paddingLeft:"10%"}}>
          <Grid.Row>
            {links(footerContent)}
            <Grid.Column width={3}>
